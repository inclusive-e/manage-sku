...
class SchemaDetector:
    ...
    COLUMN_KEYWORDS = {
        ...
        'revenue': ['revenue', 'sales', 'price', 'total', 'amount', 'value', 'turnover'],
    ...
    def detect_column_type(series: pd.Series) -> ColumnType:
        ...
        # Try boolean
        if non_null.isin([True, False, 0, 1, 'True', 'False', 'true', 'false', 'YES', 'NO', 'yes', 'no']).all():
    ...
    def detect_schema(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in df.columns:
            ...
            if suggested == 'date' and not suggested_date and col_type in [ColumnType.DATE, ColumnType.DATETIME]:
                suggested_date = col
